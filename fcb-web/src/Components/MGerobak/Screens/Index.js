...
import FormMGerobak from './Form';
import { getMasterGerobak, getMasterGerobakDetail, simpanDataGerobak, setDeleteGerobak } from "../Services/Gerobak";
        ...
        datadetail.then(response => {
            ...
            if(result.gerobak != null) {
                vdata = {
                    ...
                    isDeleted: result.gerobak.isDeleted,
        ...
        const columns = [
            ...
            },
            {
                ...
                options: {
                    ...
                    customBodyRender: (value, tableMeta, updateValue) => {
                        console.log(value);
                        let dataDeleted = (value === 1 ? "Tidak Aktif" : "Aktif");
                        let colorStat = (value === 1 ? "secondary" : "primary");
                        return (
                            <div>
        ...
        return (
            <div>
                ...
                <div className={classes.mainComponentWrapper}>
                    <Paper>
                        <MUIDatatable
                            title={"List Data Master Gerobak"}
                            data={data}
                            columns={columns}
                            options={options}
                        />
                        ...
                        >
                            <form className={classes.form} action="/" method="POST" onSubmit={(e) => { e.preventDefault(); this.submitForm();}}>
                                ...
                                <DialogContent className={classes.noPaddingDialogContent}>
                                    <div component="div" style={{ padding: 8 * 3 }}>
                                        <FormMGerobak vd = {this.state.formData} onChangeChild = {(value) => this.handleChange(value)} />
                                    </div>
                                </DialogContent>
                                <DialogActions>
